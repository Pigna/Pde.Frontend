...
function Column({table, columnName, type, defaultValue, isNullable, constraints})
{
	function getConstraintIcon(constraint)
	{
		...
		{
			case 0:
				return <FontAwesomeIcon key={constraint.name}
										icon={faKey}
										className="icon-key-primary"
										title={constraint.name}
				/>
			case 1:
				return <FontAwesomeIcon key={constraint.name}
										icon={faKey}
										className="icon-key-foreign"
										title={constraint.name}
				/>
			case 2:
	...
	function IsPrimaryOrForeignKey(constraints)
	{
		let result = false;
	...
	return (
		<tr className="column">
			...
			<td className="column-anonymize">
				{
					!IsPrimaryOrForeignKey(constraints)
					? (<AnonymizeSelector key={uuid()}
										  table={table}
										  column={columnName}
										  type={type}
					></AnonymizeSelector>)
