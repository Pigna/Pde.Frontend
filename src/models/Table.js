    ...
    return (
        <table className="table">
            ...
            <tbody className="table-columns">
                {columns.map(item => <Column key={tableName + "." + item.name} table={tableName} columnName={item.name} type={item.dataType} default={item.default} isNullable={item.isNullable} constraints={item.constraints}/>)}
            </tbody>
        </table>
    );
}
